...
- 🔍 **줌/팬 기능 추가**: 100+ 행 데이터에 마우스 휠 줌 & 드래그 이동 지원 (Chart.js Zoom Plugin)
- 🎯 **특정 칼럼 시각화**: "Category별 SalesAmount만" 같은 칼럼 지정 명령 지원
- 🎨 **범례 위치 개선**: 차트 범례를 왼쪽 상단으로 이동하여 가독성 향상
...
- ✅ **차트 시각화**: Chart.js 기반 인터랙티브 차트 생성
  ...
  - 모든 시트를 개별 차트로 시각화
  - 차트별 제목 및 범례 표시 (왼쪽 상단 정렬)
  - **줌/팬 기능**: 100+ 행 데이터에 마우스 휠 줌 & 드래그 이동 지원
  - **특정 칼럼 선택**: "Category별 SalesAmount만" 같은 명령으로 원하는 칼럼만 시각화
...
2. **자동 차트 생성**: 파일 업로드 시 자동으로 막대 그래프 생성 (명령 불필요)
   ...
   - 100+ 행 데이터는 줌/팬 기능 자동 활성화
3. **차트 유형 변경** (선택사항): "선 그래프로 보여줘", "파이 차트 만들어줘" 등의 명령
4. **특정 칼럼 시각화** (선택사항): "Category별 SalesAmount만 선 차트로" 같은 명령
5. **줌/팬 사용법** (100+ 행 차트):
   - **마우스 휠 위/아래**: 줌 인/아웃
   - **차트 드래그**: 좌우 이동
   - **더블 클릭**: 줌 초기화
...
- **chartjs-plugin-zoom** 2.0.1 - 줌/팬 인터랙션
