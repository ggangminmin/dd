...
- 🎮 **사용자 제어 기반 분석** (NEW!): Excel 파일 업로드 시 자동 분석 제거, 명령어 기반 실행
  - 📎 **파일만 첨부**: 데이터 구조와 컬럼 의미만 간단히 설명
  - 🔍 **"분석해줘" 명령**: EDA(탐색적 데이터 분석) 실행 - 히스토그램, 파이 차트, 통계표
  - 📊 **"차트 그려줘" 명령**: 막대/선/파이 차트 생성
  - ⚡ **불필요한 자동 분석 제거**: 사용자가 원할 때만 분석 실행
- 🎯 **스마트 범주형 집계**: 자연어 패턴 인식으로 데이터 자동 그룹화
  - 📝 **다양한 패턴 인식**: "Year별로", "Category로", "Customer 보고싶어" 등 자유로운 표현 지원
  - 🔍 **자동 컬럼 분류**: 범주형(categorical) vs 수치형(numeric) 컬럼 자동 감지
  - 📊 **다중 그룹화**: "Year, Category별" 같은 복합 그룹화 지원
  - ⚡ **정확한 X/Y축**: "Customer별 Revenue" → Customer(X축), Revenue(Y축) 자동 매핑
  - 💡 ID 컬럼 자동 제외 (id, _id, index, 번호 등)
  - 🎨 **미니멀 차트**: 제목/범례 제거, X/Y축 라벨만 표시
- 🧠 **스마트 데이터 집계**: 대용량 데이터(100+ 행)를 자동으로 분석하여 최적의 방식으로 요약
  - 📅 **날짜 데이터**: 월별 평균으로 자동 집계 (예: 365행 → 12행)
  - 🏷️ **카테고리 데이터**: 카테고리별 합계로 자동 집계 (중복 50% 이상)
  - 📊 **일반 데이터**: 균등 샘플링으로 100개로 축소
  - 💡 집계 정보는 차트에 자동으로 표시 (예: "월별 평균 (365개 → 12개)")
- 🥧 **파이 차트 최적화**: 500행 데이터 → 상위 10개 + 기타로 직관적 표현
- 📊 **HTML 표 형식 분석**: 데이터 분석 결과를 깔끔한 HTML 표로 표시
- 🌓 **다크모드 완벽 지원**: 차트 제목, 데이터 분석 표 모두 다크모드 최적화
- 📈 **자동 통계 표**: 합계/평균/최대/최소값을 표 형식으로 자동 생성
- 🤖 **자연어 명령 처리**: "선 차트로 나타내달라" 같은 자연어 명령 정확히 인식
...
- ✅ **Excel 자동 분석**: 업로드된 Excel 파일의 모든 시트 자동 통계 생성
  - 합계, 평균, 최대값, 최소값 자동 계산
  - 시트별 독립적인 분석 제공
- ✅ **스마트 범주형 집계** (NEW!): 자연어 패턴으로 데이터 그룹화
  - **3가지 패턴 인식**: "Year별로", "Category로", "Customer 보고싶어"
  - **자동 컬럼 분류**: 범주형 vs 수치형 자동 구분 (ID 컬럼 제외)
  - **다중 그룹화**: "Year, Category별" → Year-Category 조합으로 집계
  - **정확한 축 매핑**: "Customer별 Revenue" → X축(Customer), Y축(Revenue)
  - **수치형 필터링**: 언급된 컬럼 중 수치형은 Y축 데이터로만 사용
- ✅ **차트 시각화**: Chart.js 기반 인터랙티브 차트 생성
  - 막대 그래프, 선 그래프, 원 그래프 지원
  - 모든 시트를 개별 차트로 시각화
  - 미니멀 디자인: X/Y축 라벨만 표시, 제목/범례 제거
  - **스마트 데이터 집계**: 대용량 데이터 자동 요약 (날짜→월별 평균, 카테고리→합계, 일반→샘플링)
  - **파이 차트 스마트 축약**: 10개 초과 데이터는 상위 10개 + 기타로 자동 정리
  - **동적 너비 계산**: 막대/선 차트는 데이터 포인트당 40px 자동 할당
  - **원 차트 최적화**: 정사각형 비율 유지, 고정 600px 너비, 중앙 정렬
  - **EDA 분석 기능**: "분석해줘", "EDA" 등의 명령어로 탐색적 데이터 분석 실행
- ✅ **HTML 표 형식 분석 리포트**: 깔끔한 표 형식으로 통계 출력
  - 마크다운 대신 HTML 표로 구조화
  - 3-5행 이내로 핵심만 간결하게 요약
  - 추세 분석 (상승/하락/보합)
  - 최대 3개 데이터셋까지 자동 요약
  - 다크모드 완벽 지원
...
- **스프레드시트**: xlsx, csv (로컬 환경에서만 지원 - pandas 필요)
  ...
  - **EDA 분석**: 명령어 입력 시 각 시트별 탐색적 데이터 분석 및 차트 생성
...
2. **EDA 분석 요청**: "분석해줘", "EDA 해줘", "탐색적 분석 부탁해" 등의 명령어로 분석 시작
   - **지원 키워드**: EDA, 분석, 탐색, 데이터 분석, 통계, 시각화, 요약, analyze, analysis, explore
   - Excel/CSV 파일이 첨부되어 있어야 분석 가능
   - 자동으로 데이터 타입 분류 (수치형, 범주형, 날짜형)
   - 히스토그램, 파이 차트, 상관관계 차트 자동 생성
   - 통계 표 및 주요 인사이트 제공
3. **차트 유형 변경** (선택사항): "선 그래프로 보여줘", "파이 차트 만들어줘" 등의 명령
4. **범주형 집계 명령**: 자연어로 그룹화 기준 지정
   - "Year별로 Category와 UnitsSold" → Year별로 그룹화하여 표시
   - "Customer별 Revenue 보고싶어" → Customer별 Revenue 합계
   - "Year, Category별 SalesAmount" → 다중 그룹화 (Year-Category 조합)
   - "Region로 Total 보여줘" → Region별 Total 합계
   - 시스템이 자동으로 범주형/수치형 컬럼을 구분하여 처리
5. **결과 확인**:
   - 각 시트별로 개별 차트가 생성됩니다
   - 차트는 미니멀 디자인으로 X/Y축 라벨만 표시 (제목/범례 없음)
   - 집계된 경우 차트에 집계 정보가 자동 표시됨 (예: "월별 평균 (365개 → 12개)")
   - X축: 그룹화 기준 (예: Customer, Year-Category)
   - Y축: 수치형 데이터 (예: Revenue, SalesAmount)
   - 차트 너비는 데이터 개수에 따라 자동 조정 (포인트당 40px)
   - 파이 차트는 10개 초과 시 상위 10개 + 기타로 자동 정리
...
- **선 그래프**: "선 그래프", "꺾은선 그래프", "line chart"
  - 스마트 집계 지원 (100+ 행 데이터)
  - 시간 추이 분석에 적합 (날짜 데이터는 자동으로 월별 평균 표시)
- **원 그래프**: "원 그래프", "파이 차트", "pie chart"
  - 10개 초과 시 상위 10개 + 기타로 자동 축약
  - 전체 대비 비율, 구성비 분석에 적합
